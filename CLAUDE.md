...
├── CLAUDE.md           # this file
├── README.md
├── data/               # local corpus + manifest (payloads gitignored, downloads are manual — see README)
...
- Things that are genuinely hard to get right (HTTP clients, ASGI servers, SQL drivers, parsers, LLM SDKs, ORM, migrations, auth SDKs). Docling is the parser for this project's ingestion: it converts PDF (and HTML) directly to structured Markdown with table extraction, so don't reach for `pdfplumber`, `PyMuPDF`, or `unstructured` on top of it — check what Docling already gives you first.
