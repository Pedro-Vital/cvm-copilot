...
- `vector(1536)` columns
- generated `tsvector` columns (use the `portuguese` text search configuration for filing content, not `english`)
...
```bash
cd backend
uv run python -m ipykernel install --user --name cvm-copilot-backend --display-name "CVM Copilot Backend"
```
