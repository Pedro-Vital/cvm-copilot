...
- [x] Citation chips/links on assistant messages (company, filing type, fiscal year, page/section) — inline `[n]` markers in the rendered Markdown answer plus a "Fontes" chip row, both open the source panel
- [x] Source passage panel — show underlying excerpt for selected citation (`GET /sources/{chunk_id}` returns the cited chunk ± 1 neighbor; the panel shows the verified excerpt, the chunk in context, and a link to the filing on CVM)
...
- [x] Seed or document how to ingest/update the corpus — README "Loading and updating the corpus"; the prepared corpus (PDFs + finished Docling conversion) is shared on Google Drive so a new setup only runs `ingest.load_corpus`
